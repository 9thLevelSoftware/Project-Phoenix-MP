...
must_haves:
  ...
  key_links:
    ...
    - from: "MetricPollingEngine"
      to: "ProtocolParser"
      via: "direct function calls in polling loops"
      pattern: "parseMonitorPacket|parseDiagnosticPacket|parseHeuristicPacket"
---
...
**Job state exposure for testing:** Add an internal test helper method to MetricPollingEngine:
```kotlin
...
Tests use `engine.isJobActive(PollingType.MONITOR)` to assert job state after calling lifecycle methods. This avoids exposing raw Job references while keeping tests readable and deterministic.
...
Commit: `test(11-01): add failing tests for MetricPollingEngine lifecycle`
  ...
  <done>MetricPollingEngineTest.kt exists with 18 tests covering lifecycle (4), partial stop/Issue #222 (4), conditional restart (5), timeout disconnect/POLL-03 (3), diagnostic+heartbeat restart (2). All tests compile. Stub MetricPollingEngine.kt exists with constructor signature and isJobActive(PollingType) test helper.</done>
</task>
...
- 18 unit tests covering job lifecycle (4), Issue #222 partial stop (4), conditional restart (5), timeout disconnect (3), diagnostic+heartbeat restart (2)
