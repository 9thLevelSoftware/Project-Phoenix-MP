...
must_haves:
  truths:
    - "All UUIDs accessible via BleProtocolConstants.XXX"
...
3. Add UUID vals (Uuid objects) after existing string constants:
   ...
   - `val NUS_TX_UUID = Uuid.parse(NUS_RX_CHAR_UUID_STRING)`  // Note: TX = 6e400002 = RX_CHAR in naming
   - `val NUS_RX_UUID = Uuid.parse("6e400003-b5a3-f393-e0a9-e50e24dcca9e")`
...
2. In companion object (lines 62-147), REMOVE all constants that now exist in BleConstants:
   ...
   - Remove CONNECTION_RETRY_COUNT, CONNECTION_RETRY_DELAY_MS, CONNECTION_TIMEOUT_MS, DESIRED_MTU
...
3. Keep companion object for any state/instance-related constants that don't belong in protocol constants (if any).

4. Replace all characteristic field declarations with references to BleConstants:
   ```kotlin
