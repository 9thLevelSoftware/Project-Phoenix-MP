...
7. Add a dedicated test section for classifyTimeWindow with explicit timezone injection:
   ```kotlin
   // ========== classifyTimeWindow timezone fix (BOARD-01) ==========

   @Test
   fun classifyTimeWindowUsesLocalTime() {
       // 8am UTC = 6pm in UTC+10
       val utc8am = 1_700_006_400_000L // 2023-11-15 08:00 UTC
       val utcPlus10 = TimeZone.of("Australia/Melbourne")
       // In UTC+10, 8am UTC is 6pm local -> EVENING
   ...
   fun classifyTimeWindowNightCrossover() {
       // 11pm UTC = 10am next day in UTC+11
       val utc11pm = 1_700_060_400_000L // 2023-11-15 23:00 UTC
       val utcPlus11 = TimeZone.of("Pacific/Noumea")
       // In UTC+11, 11pm UTC is 10am next day -> AFTERNOON
       val result = SmartSuggestionsEngine.classifyTimeWindow(utc11pm, utcPlus11)
       assertEquals(TimeWindow.AFTERNOON, result)
   }
   ```

IMPORTANT: Verify the epoch millisecond values used in test data. Use `kotlinx.datetime.Instant.fromEpochMilliseconds()` to double-check that the timestamps correspond to the intended UTC times. If the hardcoded values don't align, compute correct values: Nov 15 2023 08:00 UTC = 1700035200000 (check and use correct value).
...
The existing `analyzeTimeOfDay` tests may continue to work as-is since they test the full analysis pipeline (which now uses local time internally). If any existing time-of-day test fails after the change, update it to pass an explicit TimeZone through the pipeline or adjust the expected results.
  ...
  <done>
    - classifyTimeWindow uses kotlinx-datetime Instant.toLocalDateTime with TimeZone parameter
    - classifyTimeWindow is internal (not private) for direct test access
    - Three new timezone-specific tests pass, proving local time is used
...
- classifyTimeWindow converts epoch to local time using kotlinx-datetime, not UTC modular arithmetic
