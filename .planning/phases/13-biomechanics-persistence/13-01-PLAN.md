...
<task type="auto">
  ...
  <action>
    ...
    **IMPORTANT:** The `insertSession` and `upsertSyncSession` queries MUST include all 5 new columns, and the SqlDelightWorkoutRepository mapper that reads WorkoutSession rows MUST map the new columns. Check the existing mapper in SqlDelightWorkoutRepository.kt and update it to include the new columns.
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
    ...
    **Wire biomechanics summary into saveWorkoutSession():**
    In the `saveWorkoutSession()` method, the WorkoutSession is constructed. Add the 5 new fields from the biomechanics summary:
    - Need to capture the latest biomechanics summary from the coordinator and pass it through
    - Map `BiomechanicsSetSummary` fields to WorkoutSession fields: `avgMcvMmS`, `avgAsymmetryPercent`, `totalVelocityLossPercent`, `dominantSide` (from avg asymmetry), `strengthProfile` (from avg force curve or most common)
    - The biomechanics summary is captured in handleSetCompletion AFTER saveWorkoutSession() is called — so either move the save order OR store the summary on the coordinator for the next save. Follow the existing pattern: store it on the coordinator temporarily (similar to how `setRepMetrics` works).
  </action>
  <verify>
