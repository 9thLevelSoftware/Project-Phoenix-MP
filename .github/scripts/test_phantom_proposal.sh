...
make_fake_repo() {
    ...
    mkdir -p "$repo/.github/scripts" "$repo/shared/src/iosSimulatorArm64Main/kotlin/com/devil/phoenixproject/fixture" "$repo/iosApp/PhoenixApp/PhoenixApp"
    ...
    python3 - "$repo/iosApp/PhoenixApp/PhoenixApp/Proposal.swift" "$baseline" <<'PY'
import os
import sys
from pathlib import Path
...
os.chmod(path, 0o600)
PY
    python3 - "$repo/iosApp/PhoenixApp/PhoenixApp/Binary.swift" <<'PY'
import os
import sys
from pathlib import Path
path = Path(sys.argv[1])
...
os.chmod(path, 0o600)
PY
    python3 - "$repo/iosApp/PhoenixApp/PhoenixApp/Unchanged.swift" "$repo/.gitignore" <<'PY'
import os
import sys
from pathlib import Path
...
case)
    ...
    source_text="$(cat "$REPO_ROOT/iosApp/PhoenixApp/PhoenixApp/Proposal.swift")"
    ...
    if [[ "$(pwd -P)" == "$(cd "$REPO_ROOT" && pwd -P)" && "$source_text" == *MUTATE_BASELINE* ]]; then
        printf 'mutated by baseline\n' > "$REPO_ROOT/iosApp/PhoenixApp/PhoenixApp/Original.swift"
        chmod 600 "$REPO_ROOT/iosApp/PhoenixApp/PhoenixApp/Original.swift"
    fi
    mkdir -p "$artifact"
    chmod 700 "$artifact"
...
PY2
    ...
    if [[ "$REPO_ROOT" != "$SCRIPT_REPO_ROOT" && "$source_text" == *GENERATED_RUN* ]]; then
        mkdir -p "$PWD/iosApp/PhoenixApp/build"
        printf 'generated by fake Xcode run\n' > "$PWD/iosApp/PhoenixApp/build/run-output.bin"
        chmod 600 "$PWD/iosApp/PhoenixApp/build/run-output.bin"
        mkdir -p "$PWD/iosApp/PhoenixApp/PhoenixApp.xcodeproj/project.xcworkspace"
        printf 'generated by fake Xcode run\n' > "$PWD/iosApp/PhoenixApp/PhoenixApp.xcodeproj/project.xcworkspace/contents.xcworkspacedata"
        chmod 600 "$PWD/iosApp/PhoenixApp/PhoenixApp.xcodeproj/project.xcworkspace/contents.xcworkspacedata"
    fi
    if [[ "$REPO_ROOT" != "$SCRIPT_REPO_ROOT" && "$source_text" == *GENERATED_UNKNOWN_XCODEPROJ* ]]; then
        mkdir -p "$PWD/iosApp/PhoenixApp/Unknown.xcodeproj/project.xcworkspace"
        printf 'unexpected sibling Xcode workspace\n' > "$PWD/iosApp/PhoenixApp/Unknown.xcodeproj/project.xcworkspace/contents.xcworkspacedata"
        chmod 600 "$PWD/iosApp/PhoenixApp/Unknown.xcodeproj/project.xcworkspace/contents.xcworkspacedata"
    fi
    if [[ "$REPO_ROOT" != "$SCRIPT_REPO_ROOT" && "$source_text" == *GENERATED_WORKSPACE_SYMLINK* ]]; then
        workspace="$PWD/iosApp/PhoenixApp/PhoenixApp.xcodeproj/project.xcworkspace"
        mkdir -p "$workspace"
        ln -s "$PWD/iosApp/PhoenixApp/PhoenixApp/Proposal.swift" "$workspace/unsafe-link"
    fi
    if [[ "$REPO_ROOT" != "$SCRIPT_REPO_ROOT" && "$source_text" == *MUTATE_FINAL* ]]; then
        ...
        [[ -n "$original" ]] && printf 'mutated after baseline\n' > "$original/iosApp/PhoenixApp/PhoenixApp/Original.swift"
    fi
...
if [[ "$source_text" == *GENERATED_SYMLINK* ]]; then
    ln -s "$PWD/iosApp/PhoenixApp/PhoenixApp/Proposal.swift" "$PWD/shared/build/generated-link"
fi
if [[ "$source_text" == *GENERATED_TRACKED* ]]; then
    printf '// unexpected tracked candidate mutation\n' > "$PWD/iosApp/PhoenixApp/PhoenixApp/Unchanged.swift"
    chmod 600 "$PWD/iosApp/PhoenixApp/PhoenixApp/Unchanged.swift"
fi
'''.replace('__PROPOSAL_ENV_LOG__', environment_log), encoding='utf-8')
os.chmod(path, 0o700)
PY
...
old = sys.argv[2]
new = sys.argv[3]
path.write_text(f"diff --git a/iosApp/PhoenixApp/PhoenixApp/Proposal.swift b/iosApp/PhoenixApp/PhoenixApp/Proposal.swift\n--- a/iosApp/PhoenixApp/PhoenixApp/Proposal.swift\n+++ b/iosApp/PhoenixApp/PhoenixApp/Proposal.swift\n@@ -1 +1 @@\n-{old}\n+{new}\n", encoding="utf-8")
os.chmod(path, 0o600)
PY
}
...
paths = (
    ...
    "iosApp/PhoenixApp/build/output.bin",
    "iosApp/PhoenixApp/PhoenixApp.xcworkspace/contents.xcworkspacedata",
    "iosApp/PhoenixApp/PhoenixApp/VitruvianPhoenix.xcuserdatad/UserInterfaceState.xcuserstate",
    "iosApp/PhoenixApp/DerivedData/Build/Products/app",
    "iosApp/PhoenixApp/Config/Supabase.xcconfig",
)
...
source = "iosApp/PhoenixApp/PhoenixApp/Proposal.swift"
target = "iosApp/PhoenixApp/PhoenixApp/Canonical.swift"
if operation == "rename":
...
with tempfile.TemporaryDirectory(prefix="proposal-binary-swift-") as name:
    repo = Path(name) / "repo"
    path = repo / "iosApp/PhoenixApp/PhoenixApp/Binary.swift"
...
printf '// dirty source\n' > "$IGNORED_DIRTY_REPO/iosApp/PhoenixApp/PhoenixApp/Proposal.swift"
...
for forbidden in \
    ...
    'iosApp/PhoenixApp/PhoenixApp.xcodeproj/project.pbxproj' \
    'iosApp/PhoenixApp/Config/Supabase.xcconfig' \
    'iosApp/PhoenixApp/PhoenixApp/Generated.html'; do
...
make_path_patch "$UNSUPPORTED" 'iosApp/PhoenixApp/PhoenixApp/Proposal.exe'
...
from pathlib import Path
path = Path(sys.argv[1])
path.write_text('diff --git a/iosApp/PhoenixApp/PhoenixApp/Proposal.swift b/iosApp/PhoenixApp/PhoenixApp/Proposal.swift\n--- a/iosApp/PhoenixApp/PhoenixApp/Proposal.swift\n+++ b/iosApp/PhoenixApp/PhoenixApp/Proposal.swift\n@@ -1 +1 @@\n-// baseline\n+SUPABASE_ANON_KEY=super-secret-anon-value API_TOKEN=super-secret-api-value\n', encoding='utf-8')
os.chmod(path, 0o600)
PY
...
printf '// dirty\n' > "$DIRTY_REPO/iosApp/PhoenixApp/PhoenixApp/Proposal.swift"
...
assert (repo / "iosApp/PhoenixApp/PhoenixApp/Proposal.swift").read_text() == "// baseline\n"
