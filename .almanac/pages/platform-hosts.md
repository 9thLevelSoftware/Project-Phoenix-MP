...
sources:
  ...
  - id: ios-app
    type: file
    path: iosApp/PhoenixApp/PhoenixApp/PhoenixApp.swift
...
Android applies the stored locale before `setContent {}` runs. `MainActivity.applyStoredLocaleBeforeComposition()` reads `phoenix_preferences` directly and updates the platform locale so non-English users do not see an English first frame during cold start [@main-activity].
...
iOS boot starts in Swift, not Kotlin. `PhoenixAppEntry` initializes Koin through `KoinInitIosKt.doInitKoin()`, runs migrations immediately afterward through `KoinInitKt.runMigrations()`, and then loads a SwiftUI `ContentView` that only wraps the shared Compose `UIViewController` [@ios-app] [@ios-content].
