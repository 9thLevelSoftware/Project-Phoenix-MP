...
class FakeCompletedSetRepository : CompletedSetRepository {
    ...
    private val completedSetsFlows = mutableMapOf<String, MutableStateFlow<List<CompletedSet>>>()
    ...
    private fun updateCompletedFlow(sessionId: String) {
        val sets = completedSetsBySession[sessionId]
            ...
            ?.sortedBy { it.setNumber }
            ?: emptyList()
        completedSetsFlows.getOrPut(sessionId) { MutableStateFlow(emptyList()) }.value = sets
    }
}
