...
interface WorkoutRepository {
     ...
     * transaction.
     *
     * The previous six writes were six transactions, so a process death or a
     * concurrent portal push could see a session without its sets: the push
     * acks the session and the set that lands afterwards is never re-pushed.
     *
     * Idempotent: the session is inserted only when absent, the [CompletedSet]
     * only when its id is absent, and the child tables are replaced wholesale.
     */
