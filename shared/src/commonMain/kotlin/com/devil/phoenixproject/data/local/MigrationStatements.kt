...
WHERE gs.rowid = (
    ...
    // Mirrors 49.sqm exactly: DDL only, both statements replay-safe (IF NOT EXISTS),
    // because this fallback re-runs after a partial migrate. The legacy copy is NOT
    // here -- it needs tables and healed columns the migration chain cannot guarantee,
    // so MigrationManager runs it once after the database is open. See 49.sqm.
    49 -> listOf(
