 ...
 *
 * Field names match the Echo force-config layout observed on hardware. The firmware
 * interprets these at fixed byte offsets within the 32-byte Echo command (0x4E).
 *
 * Byte layout (offsets within the Echo frame body, starting after header + rep bytes):
 *   0x06-0x07  spotter (short LE)              — always 0 on the machine
...
data class EchoParams(
    ...
    /** Reference map blend factor. Firmware offset 0x0A. Always 50 on the machine. */
    val referenceMapBlend: Int,
    /** Concentric phase delay in seconds. Firmware offset 0x0C. Always 0.1 on the machine. */
    ...
    /** Eccentric phase duration. Firmware offset 0x18. Always 0.0 on the machine. */
    val eccentricDurationSeconds: Float,
    /** Max cable velocity during eccentric phase. Firmware offset 0x1C. -200.0 on the machine. */
