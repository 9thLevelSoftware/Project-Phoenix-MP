...
class WorkoutExitPersistenceTest {
    ...
    // ===== Issue #714 (post-#716 P1 follow-up): a late async `persistSnapshot` for
    // an older captured Just Lift snapshot MUST NOT overwrite a newer synchronous
    // write for a different mode. The recency fix installs a per-snapshot ownership
    // marker (`snapshot.justLiftDefaultsPersisted`) that the synchronous completion
    // job flips on success; the async `persistSnapshot` path treats any already-
    // flipped marker as "done for this snapshot" and skips. The async fallback
    // remains intact for retained-snapshot retries where the synchronous write
    // threw and the helper cleared the marker. =====

    private fun justLiftExitSnapshot(
        completion: SetExecutionCompletion,
        ...
        val session = com.devil.phoenixproject.domain.model.WorkoutSession(
            id = lease.sessionId,
            timestamp = 0L,
            mode = ProgramMode.TUT.displayName,
            reps = 0,
            weightPerCableKg = 0f,
            progressionKg = 0f,
            duration = 0L,
        ...
        return WorkoutExitSnapshot(
            lease = lease,
            completion = completion.copy(reason = setEndReason),
            terminalPath = TerminalPath.AUTO_COMPLETE,
            session = session,
            completedSet = null,
            metrics = emptyList(),
            repMetrics = emptyList(),
            biomechanicsRepResults = emptyList(),
            singleExerciseDefaults = null,
            ...
            isRoutineSet = false,
            shouldAccumulateRoutineCalories = false,
            shouldExportIndividualHealthSession = false,
            shouldExportIndividualBackup = false,
            shouldUpdateCycleProgress = false,
            cycleId = null,
            cycleDayNumber = null,
    ...
    private fun justLiftCompletionFor(
        profileId: String,
        sessionId: String,
        executionId: Long,
        ...
        return SetExecutionCompletion(
            lease = lease,
            reason = SetEndReason.TARGET_REPS_REACHED,
            routineIdentity = null,
            attemptNumber = 1,
            acceptedDropCount = 0,
            plannedSetType = SetType.STANDARD,
            programMode = programMode,
            programmedBaseWeightPerCableKg = 0f,
            configuredStartWeightPerCableKg = 0f,
            progressionKg = 0f,
            actualReps = reps,
            targetReps = reps,
            isWarmup = false,
    ...
    @Test
    fun `Issue714 successful synchronous write makes the async fallback path no-op for the same snapshot`() = runTest {
        val harness = DWSMTestHarness(this)
        try {
            // Seed TUT defaults so the test fails fast if the persisted value flips back
            // to TUT after any subsequent step.
            ...
            advanceUntilIdle()
            val writesBefore = harness.fakeUserProfileRepo.preferenceUpdateRequests.size

            val completion = justLiftCompletionFor(
                profileId = ready.profile.id,
                sessionId = "session-sync-then-async",
                executionId = 1001L,
                programMode = ProgramMode.OldSchool,
                weightPerCableKg = 32f,
                reps = 5,
            )
            val oldSchoolDoc = JustLiftDefaultsDocument(
                workoutModeId = ProgramMode.OldSchool.modeValue,
                weightPerCableKg = 32f,
            )
            val snapshot = justLiftExitSnapshot(completion, oldSchoolDoc)

            // (a) Sync completion-job write — canonical owner for this snapshot.
            harness.activeSessionEngine
                .persistCapturedJustLiftDefaultsSnapshot(snapshot, claimIfNeeded = true)

            val writesAfterSync = harness.fakeUserProfileRepo.preferenceUpdateRequests.size
            assertEquals(
                writesBefore + 1,
                writesAfterSync,
                "sync write must perform exactly one preferences mutation",
            )
            assertEquals(
                ProgramMode.OldSchool.modeValue,
                harness.settingsManager.getJustLiftDefaultsDocument().workoutModeId,
            )
            assertTrue(
                snapshot.justLiftDefaultsPersisted,
                "marker must be flipped after successful sync write",
            )

            // (b) Late-arriving async coroutine for the SAME snapshot — must no-op.
            harness.activeSessionEngine
                .persistCapturedJustLiftDefaultsSnapshot(snapshot, claimIfNeeded = false)

            assertEquals(
                writesAfterSync,
                harness.fakeUserProfileRepo.preferenceUpdateRequests.size,
                "async fallback must NOT write again once the sync owner flipped the marker",
            )
            assertTrue(
                snapshot.justLiftDefaultsPersisted,
                "marker must remain true after the no-op async fallback",
            )
            assertEquals(
                ProgramMode.OldSchool.modeValue,
                harness.settingsManager.getJustLiftDefaultsDocument().workoutModeId,
                "async fallback must leave the freshly persisted Old School value untouched",
            )
        } finally {
            harness.cleanup()
        }
    ...
    @Test
    fun `Issue714 sync failure preserves the async fallback so the snapshot can still persist`() = runTest {
        val harness = DWSMTestHarness(this)
        try {
            val ready = harness.fakeUserProfileRepo.activeProfileContext.value
                as com.devil.phoenixproject.data.repository.ActiveProfileContext.Ready
            ...
            val completion = justLiftCompletionFor(
                profileId = ready.profile.id,
                sessionId = "session-sync-then-retry",
                executionId = 2002L,
                programMode = ProgramMode.OldSchool,
                weightPerCableKg = 32f,
                reps = 5,
            ...
            // (a) Sync write FAILS — the catch block must clear the marker so the async
            // retry can still take ownership of this snapshot.
            harness.fakeUserProfileRepo.updateWorkoutFailure = RuntimeException("transient sync failure")
            val syncAttempt = runCatching {
                harness.activeSessionEngine
                    .persistCapturedJustLiftDefaultsSnapshot(snapshot, claimIfNeeded = true)
            }
            assertTrue(syncAttempt.isFailure, "sync attempt must propagate the injected failure")
            assertFalse(
                snapshot.justLiftDefaultsPersisted,
                "marker must be cleared after the sync write threw, so async retry can take ownership",
            )

            // (b) Clear the injected failure and invoke the async fallback (claimIfNeeded =
            // false). The marker is false → the helper must write and flip the marker.
            ...
            assertTrue(
                snapshot.justLiftDefaultsPersisted,
                "marker must be flipped after the async fallback succeeds",
            )
            assertEquals(
                ProgramMode.OldSchool.modeValue,
                harness.settingsManager.getJustLiftDefaultsDocument().workoutModeId,
                "retained-snapshot async retry must persist Old School after a sync failure",
            )
            val writesAfter = harness.fakeUserProfileRepo.preferenceUpdateRequests.size
            assertEquals(
                writesBefore + 1,
                writesAfter,
                "failure recovery must produce exactly one preferences mutation (the async retry)",
            )
        } finally {
            harness.fakeUserProfileRepo.updateWorkoutFailure = null
            harness.cleanup()
    ...
    @Test
    fun `Issue714 interleaved completion — older async TUT snapshot's late write does not overwrite newer Old School sync write`() = runTest {
        // Deterministic test of the exact in-the-wild race the post-#716 audit
        // flagged in discussion_r3837465213: an older snapshot's async
        // `persistSnapshot` reaches `mutateWorkout` AFTER a newer snapshot's
        // synchronous write has already persisted fresh Just Lift defaults. Both
        // writers go through the same preferences mutex. Without per-snapshot
        // ownership, the older coroutine would clobber the fresh defaults with
        // its STALE captured TUT values. The fix makes the sync write canonical
        // per snapshot, and each async call is gated on a per-snapshot marker so
        // redundant writes (regardless of the captured Just Lift defaults) are
        // suppressed.
        val harness = DWSMTestHarness(this)
        try {
            val ready = harness.fakeUserProfileRepo.activeProfileContext.value
            ...
            val olderCompletion = justLiftCompletionFor(
                profileId = ready.profile.id,
                sessionId = "older-TUT-session",
                executionId = 9001L,
                programMode = ProgramMode.TUT,
                weightPerCableKg = 20f,
                reps = 5,
            )
            val olderSnapshot = justLiftExitSnapshot(
                olderCompletion,
            ...
            val newerCompletion = justLiftCompletionFor(
                profileId = ready.profile.id,
                sessionId = "newer-OldSchool-session",
                executionId = 9002L,
                programMode = ProgramMode.OldSchool,
                weightPerCableKg = 32f,
                reps = 3,
            )
            val newerSnapshot = justLiftExitSnapshot(
                newerCompletion,
                JustLiftDefaultsDocument(
                    workoutModeId = ProgramMode.OldSchool.modeValue,
                    weightPerCableKg = 32f,
            ...
            // Step 1 — older (TUT) sync write succeeds and flips its marker.
            harness.activeSessionEngine
                .persistCapturedJustLiftDefaultsSnapshot(olderSnapshot, claimIfNeeded = true)
            val afterOlderSync = harness.fakeUserProfileRepo.preferenceUpdateRequests.size
            assertEquals(writesBefore + 1, afterOlderSync)
            assertTrue(olderSnapshot.justLiftDefaultsPersisted)

            // Step 2 — older (TUT) async coroutine runs for the SAME older snapshot.
            // The per-snapshot marker is true, so it must no-op.
            harness.activeSessionEngine
                .persistCapturedJustLiftDefaultsSnapshot(olderSnapshot, claimIfNeeded = false)
            assertEquals(
                afterOlderSync,
                harness.fakeUserProfileRepo.preferenceUpdateRequests.size,
                "older async fallback must no-op after older sync already persisted",
            )

            // Step 3 — newer (Old School) sync write takes ownership of its own snapshot.
            harness.activeSessionEngine
                .persistCapturedJustLiftDefaultsSnapshot(newerSnapshot, claimIfNeeded = true)
            assertEquals(
                afterOlderSync + 1,
                harness.fakeUserProfileRepo.preferenceUpdateRequests.size,
                "newer sync write must produce exactly one preferences mutation",
            )
            assertTrue(newerSnapshot.justLiftDefaultsPersisted)

            // Primary user-visible assertion: the persisted Just Lift defaults reflect
            // the NEWER Old School selection, not the older TUT capture. A regression
            // that re-enables the stale-overwrite race would flip this back to TUT
            // after a hypothetical older async that ran AFTER step 3.
            val persisted = harness.settingsManager.getJustLiftDefaultsDocument()
            assertEquals(
                ProgramMode.OldSchool.modeValue,
                persisted.workoutModeId,
                "Newer Old School write must remain durable. " +
                    "Older snapshot captured TUT defaults with weightPerCableKg=20f; " +
                    "this is the staleness-defence assertion the audit flagged (PR #716 discussion_r3837465213).",
            )
            assertEquals(32f, persisted.weightPerCableKg)

            // Per-snapshot marker isolation: the older marker is true (sync wrote),
            // the newer marker is true (sync wrote). They must not interfere — a
            // shared marker would either lose the per-snapshot isolation or
            // overwrite one snapshot's value with another.
            assertTrue(olderSnapshot.justLiftDefaultsPersisted)
            assertTrue(newerSnapshot.justLiftDefaultsPersisted)
        } finally {
            harness.cleanup()
