...
- DEFERRED (parity-sensitive, no change): F087/F348 (DiagnosticFaultDecoder) — an unknown-bits/label-disambiguation rewrite was prototyped but reverted: it altered user-visible diagnostic label strings (e.g. "Message failure (8)") that existing tests and the portal-parity contract assert verbatim. Cosmetic, not a correctness bug; left as-is.
