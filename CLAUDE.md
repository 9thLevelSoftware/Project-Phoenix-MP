...
- The stored **per-profile training max** (`ExerciseTrainingMax`, migration 49: manual 5/3/1 input, a VBT assessment, or a claimed legacy value) is the fallback scaling baseline for `% of PR` routines when no matching PersonalRecord exists (`ResolveRoutineScalingBaselineUseCase`, `ResolveRoutineWeightsUseCase`). It is read and written ONLY through `ExerciseRepository.getTrainingMax` / `setTrainingMax`, always with a `profileId`.
  - `Exercise.one_rep_max_kg` is a **legacy column**. It has no field on the `Exercise` domain model and nothing writes it; only migration 49 and its post-open repair read it. Do not reintroduce a global 1RM.
  - **Saving a PR never writes a training max.** A PR is a separate metric, and one profile's PR must never move another profile's commanded load or overwrite a manually entered training max.
  - A pre-migration-49 value whose owner could not be determined stays **unassigned** — never assigned to `default` — until a profile claims it through the one-time prompt on the training-max input.
